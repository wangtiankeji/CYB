package com.greatwall.platform.base.interceptor;
...
/**   
 * @Description: mybatis 重写sql语句实现物理分页
 * @author fudk fudk_k@sina.com   
 * @date 2014年7月26日 下午8:24:13    
 */
...
public class PageInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        /*//SQL 打印start
        BoundSql boundSqlp = (BoundSql) metaStatementHandler.getValue("delegate.boundSql");
        Object parameterObjectp = boundSqlp.getParameterObject();
        if (parameterObjectp != null) {
            String sql = boundSqlp.getSql();
...
*/        
     ...
     * @Description: 从数据库里查询总的记录数并计算总页数，回写进分页参数PageParameter,这样调用者就可用通过 分页参数PageParameter获得相关信息。
     * @param @param sql
...
//            logger.error("Ignore this exception", e);
            throw new SQLException(countSql,e);
        } finally {
            try {
            	if(rs!=null){
            		rs.close();
    ...
    /**
     * @Title: buildPageSql
     * @Description: 根据数据库类型，生成特定的分页sql
     * @param @param sql
     * @param @param page
     * @param @return    设定文件
     * @return String    返回类型
